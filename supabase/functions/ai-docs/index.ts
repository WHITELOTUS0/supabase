...
interface RequestData {
  service: 'openai' | 'unify';
  messages: Message[];
}

const openAiKey = Deno.env.get('OPENAI_API_KEY');
const unifyKey = Deno.env.get('UNIFY_API_KEY');
...
serve(async (req) => {
  try {
    ...
    const { service, messages } = requestData;
    ...
    if (service === 'unify') {
      if (!unifyKey) {
        throw new ApplicationError('Missing environment variable UNIFY_API_KEY');
      }
      ...
      const response = await fetch('https://api.unify.ai/v0/chat/completions', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new ApplicationError('Failed to generate completion with Unify', error);
      }
    ...
    throw new UserError('Invalid service specified');
  } catch (err: unknown) {
